...
fun MintPermissionsController.observe(vararg permissions: MintPermission): Flow<List<MintPermissionStatus>> {
    return observe(permissions.asList())
}

suspend fun MintPermissionsController.get(vararg permissions: MintPermission): List<MintPermissionStatus> {
